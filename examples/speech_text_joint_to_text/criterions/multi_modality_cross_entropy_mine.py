...
@register_criterion(
    "speech_text_pretrain_cross_entropy_mine",
    dataclass=LabelSmoothedCrossEntropyCriterionConfig,
)
class SpeechTextPreTrainCrossEntCriterionMine(LabelSmoothedCrossEntropyCriterion):
    ...
    def get_lprobs_and_target(self, model, net_output, sample):
        ...
        if mode == "unsup_speech":
            lprob = utils.log_softmax(net_output[2], dim=-1, onnx_trace=self.onnx_trace)
            lprob.batch_first = True
            lprobs = [model.get_normalized_probs(net_output, log_probs=True), lprob)
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        if isinstance(target, dict):
            ...
            if not lprobs[0].batch_first:
                lprob = lprobs[0].transpose(0, 1)
                t_lprobs = t_lprobs.transpose(0, 1)
            nsentences, seq_len = lprob.size()[:2]
            ...
            loss = (
                -(t_probs * (lprob - t_lprobs)).sum()
                if reduce
                else -(t_probs * (lprob - t_lprobs)).sum(-1, keepdim=True)
            )
            
            t_lprobs = target["target_logprobs1"]

            if not lprobs[1].batch_first:
                lprob1 = lprobs[1].transpose(0, 1)
                t_lprobs = t_lprobs.transpose(0, 1)
            nsentences, seq_len = lprob1.size()[:2]
            ntokens = nsentences * seq_len
            t_probs = t_lprobs.exp()
            mask_indices = (
                net_output[1]["mask_indices"][0]
            ...
            t_lprobs = t_lprobs.detach()
            loss1 = (
                -(t_probs * (lprob1 - t_lprobs)).sum()
                if reduce
                else -(t_probs * (lprob1 - t_lprobs)).sum(-1, keepdim=True)
            )
            
            
            nll_loss = 0.5*loss + 0.5*loss1
        else:
